...
const useSpeechText = ((dependency:any,text:string|null,voice_index:number)=>{
    const femaleVoicesIndex = useRef<number[]>([21 , 22, 23, 24, 26, 32, 35, 37, 41,42])
    const [speaking,setSpeaking] = useState<boolean>(false);
    useEffect(()=>{
        if(text){
            console.log("inside useffect",dependency,text);
